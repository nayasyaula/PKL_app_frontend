...
const HomeScreen = () => {
  ...
  const [cardInTime, setCardInTime] = useState('N/A');
  const [cardOutTime, setCardOutTime] = useState('N/A');
  ...
  const fetchTodayAttendance = async (token) => {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        setTodayAttendanceData(todayAttendance); // Update state with today's data
      }
    } catch (error) {
  ...
  const fetchAttendance = async (token) => {
    try {
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        setAttendanceData([]); // Update state
      }
    } catch (error) {
      console.error('Failed to fetch attendance data:', error);
    }
  ...
  const getIconAndColor = (status, type) => {
    if (type === 'in') {
      return {
  ...
  const getStatusTextColor = (status, type) => {
    if (type === 'in') {
